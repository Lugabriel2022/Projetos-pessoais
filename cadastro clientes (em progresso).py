...
class Application(Funcs):
    def __init__(self):  # cira a função de inicialização da janela
        self.jan = (
            jan  # gera uma equivalencia lembrando o programa que jan se refere a jenela
        )
    ...
    def tela(self):  # cria a função tela
        ...
        self.jan.configure(
            background="#1e3743"
        )  # define a cor do backgtound da janela usado códigos hexa decimais ou cores pré setadas
        self.jan.geometry(
            "700x500"
        )  # define o tamanho inicial da janela, sendo sempre (largura x altura)
        self.jan.resizable(True, True)  # define se a janela sera responsiva ou não
        self.jan.maxsize(
            width=900, height=700
        )  # define o tamanho limite da tela, sendo sempre (largura x altura)
        self.jan.minsize(
            width=600, height=400
        )  # define o tamanho minimo da tela, sendo sempre (largura x altura)
    ...
    def frame_da_tela(self):
        ...
        )  # define o tamanho, posição e coloca o frame na janela
    ...
    def widget_frame1(self):  ###Criação do botão limpar
        ...
        self.bt_limpar.place(
            relx=0.2, rely=0.1, relwidth=0.1, relheight=0.15
        )  # botão buscar
        ...
        self.bt_buscar.place(
            relx=0.3, rely=0.1, relwidth=0.1, relheight=0.15
        )  # botão novo
        ...
        self.bt_apagar.place(
            relx=0.8, rely=0.1, relwidth=0.1, relheight=0.15
        )  # Criação da label e entrafa do codigo
        self.lb_codigo = tk.Label(
            ...
            fg="#00008B",
            font=("Comic Sans MS", 8, "bold"),
        )  # bg é a cor do fundo, fg é a cor do texto, font é a fonte do texto
        ...
        self.lb_nome.place(
            relx=0.05, rely=0.35
        )  # rel significativa relativo, ou seja relx significa que a posição é relativa a posição x da janela ou no caso do frame
        self.nome_entry = tk.Entry(self.frame_1)
        self.nome_entry.place(
            relx=0.05, rely=0.45, relwidth=0.8
        )  # relwidth quer dizer que a largura é relativaa a lagura do frame
        # #criação label e entrada telefone
        self.lb_tel = tk.Label(
            self.frame_1,
            text="Telefone",
            bg="#dfe3ee",
            fg="#00008B",
    ...
    def widget_frame2(self):
        ...
        self.listacli = ttk.Treeview(
            self.frame_2, height=4, column=("col1", "col2", "col4", "col4")
        )  # ttk é um modulo avançado do tkinter e deve ser importado separadamente, a treeview ´w uma lsita simples no questio que existem mais avançados
